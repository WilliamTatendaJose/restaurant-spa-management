...
export default function DatabaseSettingsPage() {
  ...
  const handleFullReset = () =>
    runOperation(resetAndResync, 'Database reset and resync');
  const handleManualSync = () => runOperation(manualSync, 'Manual sync');
  const handlePushChanges = () =>
    runOperation(pushChanges, 'Push local changes');
  const handlePullChanges = () =>
    runOperation(pullChanges, 'Pull server changes');
                  ...
                  <div className='space-y-2'>
                    ...
                    {result.details?.tables && (
                      <div className='mt-2'>
                        ...
                        <ul className='ml-4 text-sm opacity-75'>
                          {Object.entries(result.details.tables).map(
          ...
          <CardContent className='space-y-3'>
            <div className='space-y-2 text-sm'>
              ...
              <ul className='list-inside list-disc space-y-1 text-muted-foreground'>
                <li>
                  Try "Bidirectional Sync" first to resolve most sync conflicts
                </li>
                <li>
                  If data appears inconsistent, use "Reset Database" to get a
                  fresh copy from server
                </li>
                <li>
                  Use "Push Changes" when you have local changes that need to be
                  uploaded
                </li>
                <li>
                  Use "Pull Changes" to get the latest data from server without
              ...
              <ul className='list-inside list-disc space-y-1 text-muted-foreground'>
                ...
                <li>
                  User authentication works across browsers, but data sync
                  requires login
                </li>
                <li>
                  If sync fails with "Authentication required", try logging out
                  and back in
                </li>
              </ul>
            </div>
