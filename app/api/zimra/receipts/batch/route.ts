...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dailyTransactions.length === 0) {
      return NextResponse.json(
        { message: "No completed transactions found for the specified date", receiptsSubmitted: 0 },
        { status: 200 }
      )
    }
    ...
    if (result.success) {
      console.log(`ZIMRA batch receipt submission successful for ${date}: ${zimraReceipts.length} receipts`)
