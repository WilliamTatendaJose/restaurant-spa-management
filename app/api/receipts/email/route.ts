...
async function sendWithResend(request: NextRequest) {
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <div style="padding: 20px;">
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${receiptUrl}" target="_blank" style="background-color: #6b7280; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block;">View Receipt</a>
        </div>
  ...
  return new NextResponse(
    JSON.stringify({ 
      success: true, 
      message: "Receipt email sent successfully via Resend",
      emailId: data?.id
    }),
...
async function sendWithSendGrid(request: NextRequest) {
  ...
  return new NextResponse(
    JSON.stringify({ 
      success: true, 
      message: "Receipt email sent successfully via SendGrid" 
    }),
    { status: 200, headers: { "content-type": "application/json" } }
  );
}