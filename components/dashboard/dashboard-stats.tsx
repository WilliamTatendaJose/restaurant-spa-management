...
import { bookingsApi, customersApi, spaServicesApi, transactionsApi } from "@/lib/db"
    ...
    async function fetchStats() {
      try {
        ...
        const prevMonthTransactions = completedTransactions.filter(tx => {
          const txDate = new Date(tx.transaction_date)
          return txDate < lastMonthDate && txDate >= new Date(lastMonthDate.getTime() - 30 * 24 * 60 * 60 * 1000)
        })
        ...
        const prevWeekBookings = allBookings.filter(booking => {
          const bookingDate = new Date(booking.booking_date)
          return bookingDate < lastWeekDate && bookingDate >= new Date(lastWeekDate.getTime() - 7 * 24 * 60 * 60 * 1000)
        })
        
        // Calculate booking change percentage
        const bookingsChange = prevWeekBookings.length === 0 
          ? 100 
          : ((thisWeekBookings.length - prevWeekBookings.length) / prevWeekBookings.length) * 100
        ...
        // Calculate orders change percentage
        const spaOrdersChange = yesterdaySpaOrders.length === 0
          ? 100
          : ((todaySpaOrders.length - yesterdaySpaOrders.length) / yesterdaySpaOrders.length) * 100
        ...
        <CardContent>
          {isLoading ? (
            ...
            <>
              ...
              <p className={`text-xs ${stats.bookingsChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <>
              <div className="text-2xl font-bold">+{stats.restaurantOrdersCount}</div>
              <p className={`text-xs ${stats.ordersChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <>
              ...
              <p className={`text-xs ${stats.spaOrdersChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                {formatChange(stats.ordersChange)} from yesterday
              </p>
            </>
          )}
