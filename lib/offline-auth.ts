...
async function ensureAuthStoreExists() {
  try {
    ...
    if (!initialized) {
      ...
      console.log('Creating users with emails:', [adminUser.email, managerUser.email, staffUser.email]);
      
      // Initialize with all test users
      await storage.setItem<UserWithHash[]>('users', [adminUser, managerUser, staffUser]);
