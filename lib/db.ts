...
export async function listRecords(
  ...
  const shouldSync =
    force || !lastSync[table] || Date.now() - lastSync[table] > 5 * 60 * 1000;
...
export const transactionsApi = {
  ...
  getItems: async (transactionId: string) => {
    ...
    return memoryDb['transaction_items'].filter(
  ...
  getDailyRevenue: async (days: number = 7) => {
    ...
    let completedTransactions =
      optimizer.queryByIndex('transactions', 'status', 'completed') || [];
    const paidTransactions =
      optimizer.queryByIndex('transactions', 'status', 'paid') || [];
    ...
    type RevenueData = {
      [key: string]: { spa: number; restaurant: number; name: string };
    };
    ...
    completedTransactions.forEach((transaction) => {
      ...
      if (dateSet.has(date)) {
        const serviceType =
          transaction.transaction_type?.toLowerCase() || 'restaurant';
...
export const menuItemsApi = {
  ...
  listByCategory: async (category: string) => {
    ...
    return items.filter(
      (item: any) => item.status === 'active' || item.isAvailable !== false
    );
  },
  listActive: async () => {
...
export const businessSettingsApi = {
  ...
  update: (id: string, data: any) =>
    updateRecord('business_settings', id, data),
  ...
  getSettings: async (defaultSettings: any = {}) => {
    ...
    const newSettings = await createRecord(
      'business_settings',
      defaultSettings
    );
    return newSettings;
...
export const feedbackApi = {
  ...
  getAverageRating: async () => {
    ...
    const totalRating = memoryDb.feedback.reduce(
...
// Mock API for tasks since it's not in the core database schema
export const tasksApi = {
  async get(id: string): Promise<any | null> {
    // First, try to get from memoryDb
    const taskInMemory = memoryDb.tasks.find((t) => t.id === id);
    if (taskInMemory) {
      return taskInMemory;
    }

    // If not in memory, try IndexedDB
    const record = await getRecord('tasks', id);
    if (record) {
      return record;
    }

    // If not found, create a mock task for demonstration
    if (!record) {
      const mockTask = {
        id: id,
      ...
      // Add to memoryDb for future requests
      memoryDb.tasks.push(mockTask);
      await addToIndexedDB('tasks', mockTask);
      return mockTask;
    }
  ...
  async list(filters: any = {}): Promise<any[]> {
    return listRecords('tasks', filters);
  },

  async create(data: any): Promise<any> {
    return createRecord('tasks', data);
  },

  async update(id: string, data: any): Promise<any> {
    return updateRecord('tasks', id, data);
  },
