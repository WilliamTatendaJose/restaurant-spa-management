...
const TABLE_VALIDATION_RULES: Record<
  string,
  {
    required?: string[];
...
function validateAndCleanRecord(record: any, tableName: string): any {
  ...
  Object.keys(rules.enums || {}).forEach((field) => {
    ...
    if (cleaned[field] && !rules.enums![field].includes(cleaned[field])) {
      console.warn(
        `Invalid ${field} value '${cleaned[field]}' in ${tableName}, using default`
      );
  ...
  if (tableName === 'menu_items') {
    ...
    if (cleaned.dietary && Array.isArray(cleaned.dietary)) {
      cleaned.dietary = cleaned.dietary.filter(
    ...
    if (
      !cleaned.category ||
      !validCategories.includes(cleaned.category.toLowerCase())
    ) {
      console.warn(
        `Invalid category '${cleaned.category}' for menu item '${cleaned.name}', setting to 'other'`
      );
    ...
    Object.keys(cleaned).forEach((key) => {
      if (!allowedFields.includes(key)) {
        console.warn(
          `Removing unknown field '${key}' from menu item '${cleaned.name}'`
        );
        delete cleaned[key];
      }
    });
  ...
  if (tableName === 'transactions') {
    ...
    ['subtotal', 'total_amount', 'tax_amount', 'tip_amount'].forEach(
      (field) => {
...
export class DatabaseSync {
  ...
  async resetAndResync(): Promise<SyncResult> {
    ...
    try {
        ...
        try {
          ...
          if (error) {
            ...
            if (
              ...
              console.log(
                `📝 ${tableName}: Table does not exist in Supabase yet`
              );
              results[tableName] = {
                count: 0,
                note: 'Table not found in Supabase',
              };
            }
            continue;
          }
          ...
          if (serverData && serverData.length > 0) {
            ...
            console.log(
              `✅ ${tableName}: ${serverData.length} records synced from Supabase`
            );
          } else {
      ...
      if (totalSynced === 0) {
        console.log(
          '📋 No data found on server, adding sample data locally...'
        );
      ...
      console.log(
        `🎉 Reset and resync completed: ${totalSynced} total items from Supabase`
      );
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : 'Reset and resync failed',
      };
    } finally {
    ...
    try {
      ...
      for (const table of SYNC_TABLES) {
        ...
        if (localRecords.length > 0) {
          console.log(
            `Syncing ${localRecords.length} ${table} records to server...`
          );

          // Clean and validate records before syncing
          let cleanedRecords = localRecords.map((record: any) =>
            validateAndCleanRecord(record, table)
          );

          // Deduplicate records for menu_items based on name and category
          if (table === 'menu_items') {
            cleanedRecords = cleanedRecords.reduce(
              (acc: any[], current: any) => {
            ...
            console.log(
              `Deduplicated ${localRecords.length} records to ${cleanedRecords.length} for ${table}`
            );
          }
          ...
          try {
            ...
            if (error) {
              ...
              for (const record of cleanedRecords) {
                try {
                  ...
                  if (singleError) {
                    console.error(
                      `Failed to sync individual record in ${table}:`,
                      {
                        recordId: record.id,
                  ...
                  console.error(
                    `Exception syncing individual record in ${table}:`,
                    {
                      recordId: record.id,
                      recordName: record.name || 'Unknown',
                      recordData: record, // Log the actual record data for debugging
                      error:
                        singleSyncError instanceof Error
                          ? singleSyncError.message
                          : String(singleSyncError),
                    }
                  );
              ...
              if (successCount > 0) {
                console.log(
                  `✓ Successfully synced ${successCount}/${cleanedRecords.length} ${table} records`
                );
              ...
              if (failedRecords.length > 0) {
                console.warn(
                  `⚠️ Failed to sync ${failedRecords.length}/${cleanedRecords.length} ${table} records`
                );
                // Don't throw error if some records succeeded
                if (successCount === 0) {
                  throw new Error(
                    `Failed to sync all ${table} records. Check console for detailed errors.`
                  );
                }
              }
            } else {
              console.log(
                `✓ Successfully synced ${cleanedRecords.length} ${table} records`
              );
            ...
            throw new Error(
              `Failed to sync ${table}: ${syncError instanceof Error ? syncError.message : String(syncError)}`
            );
          }
        }
      }
    ...
    try {
      ...
      for (const tableName of SYNC_TABLES) {
        ...
        if (serverRecords && serverRecords.length > 0) {
          console.log(
            `📥 Processing ${serverRecords.length} ${tableName} records from Supabase...`
          );

          for (const serverRecord of serverRecords) {
            try {
              // Check if record exists locally
              const localRecord = await listRecords(tableName, {
                id: serverRecord.id,
              });
              ...
              if (localRecord.length === 0) {
                ...
                totalPulled++;
                console.log(
                  `✅ Added new ${tableName}: ${serverRecord.name || serverRecord.id}`
                );
              } else {
                ...
                if (serverUpdated > localUpdated) {
                  ...
                  totalPulled++;
                  console.log(
                    `✅ Updated ${tableName}: ${serverRecord.name || serverRecord.id}`
                  );
                }
              }
            } catch (error) {
              console.error(
                `❌ Error processing server record for ${tableName}:`,
                error
              );
            }
          }
      ...
      console.log(
        `📥 Successfully pulled ${totalPulled} records from Supabase`
      );
  ...
  async bidirectionalSync(): Promise<SyncResult> {
    ...
    try {
      ...
      console.log(
        `🎉 Bidirectional sync completed: ${totalSynced} items synced`
      );
...
export const suppressSyncToasts = (duration?: number) =>
  databaseSync.suppressToasts(duration);
