...
export class ZIMRAApiClient {
  ...
  async submitReceiptsBatch(receipts: ZIMRAReceipt[]): Promise<ZIMRAResponse> {
    try {
      ...
      return {
        success: true,
        message: responseData.message || `${receipts.length} receipts submitted successfully`,
        ...
        declaration: {
          ...
          business_breakdown: [
            ...
            },
            {
              ...
              transaction_count: dailyReturn.breakdown.restaurant.transactionCount
            }
          ]
        },
        submission_timestamp: new Date().toISOString()
  ...
  async getSubmissionStatus(referenceNumber: string): Promise<any> {
    try {
      ...
      const response = await fetch(`${this.config.apiUrl}/returns/status/${referenceNumber}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
  ...
  static generateFiscalCode(taxNumber: string, receiptNumber: string, dateTime: Date): string {
  ...
  static transactionToZIMRAReceipt(
    ...
    const fiscalCode = this.generateFiscalCode(taxNumber, transaction.id, dateTime)
    ...
    const items = transactionItems.map(item => {
      ...
      return {
        ...
        category: (transaction.transaction_type === 'spa' ? 'SPA_SERVICES' : 'RESTAURANT_SERVICES') as 'SPA_SERVICES' | 'RESTAURANT_SERVICES'
      }
    })
...
export function calculateZimbabweVAT(amount: number, vatRate: number = 14): { vatAmount: number, netAmount: number } {
...
export function validateZIMRAReceipt(receipt: ZIMRAReceipt): { valid: boolean, errors: string[] } {
  const errors: string[] = []
